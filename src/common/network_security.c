...
ssize_t ssds_read(int fd, void *buf, size_t count)
{
	...
	{
		ssds_log(logDEBUG, "Using nonciphered read\n");
		res = read(fd, buf, count);
	}
	else
	{
		ssds_log(logDEBUG, "Using ciphered read\n");
...
ssize_t ssds_write(int fd, const void *buf, size_t count)
{
	...
	if (_use_secure_connection == 0)
	{
		ssds_log(logDEBUG, "Using nonciphered write\n");
		res = write(fd, buf, count);
	}
	else
	{
		ssds_log(logDEBUG, "Using ciphered write\n");
...
int initialize_secure_connection(int fd)
{
	...
	{
		ssds_log(logERROR, "SSL fail (server load certs), using non-ciphered connection\n");
		//ERR_print_errors_fp(stderr);
	...
	{
		ssds_log(logERROR, "SSL fail (client connect), using non-ciphered connection\n");
		ERR_print_errors_fp(stderr);
		_use_secure_connection = 0;
		return -1;
	}
	...
	{
		ssds_log(logMESSAGE, "Successfully connected\n");
	}
	#endif
	...
	if (SSL_accept(ssl) == -1)
	{
		ssds_log(logERROR, "SSL fail (server accept), using non-ciphered connection\n");
		ERR_print_errors_fp(stderr);
		_use_secure_connection = 0;
		return -1;
	}
	...
	{
		ssds_log(logMESSAGE, "Successfully accepted\n");
	}
	#endif
...
{ 
	...
	{
		ssds_log(logERROR, "Couldnt use any ciphers, switching to non-ciphered connection\n");
		_use_secure_connection = 0;
	}
	if ( ctx == NULL )
	{
...
int ShowCerts(SSL* ssl)
{
	...
	if ( cert != NULL )
	{
		ssds_log(logDEBUG, "Server certificates:\n");
		line = X509_NAME_oneline(X509_get_subject_name(cert), 0, 0);
		ssds_log(logDEBUG, "Subject: %s\n", line);
		free(line);
		line = X509_NAME_oneline(X509_get_issuer_name(cert), 0, 0);
		ssds_log(logDEBUG, "Issuer: %s\n", line);
		free(line);
		X509_free(cert);
		return 0;
	}
	else
		ssds_log(logERROR, "No peer certificates avalaible.\n");
	return -1;
}
...
int LoadCertificates(SSL_CTX* ctx, char* CertFile, char* KeyFile)
{
    ...
    {
        //ERR_print_errors_fp(stderr);
        ssds_log(logERROR, "Couldn load certificate\n");
        return -1;
    }
    if (SSL_CTX_use_PrivateKey_file(ctx, KeyFile, SSL_FILETYPE_PEM) <= 0)
    {
        //ERR_print_errors_fp(stderr);
        ssds_log(logERROR, "Couldnt load private key\n");
        return -1;
    }
