w/* Server side dependency solving - transfer of dependency solving from local machine to server when installing new packages
...
int resolve_dependency_file_path(char * ret_val, char** arch, char** release) {
    ...
    *release = NULL;
    ...
    if(uname(machine) < 0) {
        syslog(LOG_ERR, "Unable to retreive system info");
        fprintf(stderr, "ERROR: Unable to retreive system information.\n");
        free(machine);
        return 1;
    }

    char *end = strchr(machine->release, '.');
...
void rds_signal_handler(int signum) {
    syslog(LOG_ERR, "Ended with signal %d", signum);
